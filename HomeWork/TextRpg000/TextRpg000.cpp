...
void StatusRender(const char* _Name, int _Att, int _HP, int _Speed)
{
...
{
    StatusRender(PlayerName, PlayerAtt, PlayerHp, PlayerSpeed);
}

// 편의를 위해서 랩핑했다고 한다.
void MonsterStatusRender()
{
    StatusRender(MonsterName, MonsterAtt, MonsterHp, MonsterSpeed);
}
...
int main()
{
    ...
    while (true)
    {
        ...
        else if(MonsterSpeed > PlayerSpeed)
        {
            system("cls");
            DamageLogic(PlayerHp, MonsterAtt);
            PlayerStatusRender();
